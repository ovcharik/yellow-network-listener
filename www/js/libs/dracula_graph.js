...
Graph.prototype = {
    ...
    addEdge: function(source, target, style) {
        ...
        edge.port = style.port;
        edge.node = style.node;
...
Graph.Renderer.Raphael = function(element, graph, width, height) {
    ...
    this.radius = 60; /* max dimension of a node */
    this.graph = graph;
    this.mouse_in = false;
    ...
    this.dragger = function (e) {
        ...
        this.dy = e.clientY;
        selfRef.isDrag = this;
        //this.set && this.set.animate({"opacity": .5}, 200);// && this.set.toFront();
    ...
    d.onmousemove = function (e) {
        e = e || window.event;
        if (selfRef.isDrag) {
            var ds = selfRef.graph.scale.current;
            selfRef.isMove = true;
            ...
            selfRef.isDrag.set.translate((clientX - Math.round(selfRef.isDrag.dx)) / ds, (clientY - Math.round(selfRef.isDrag.dy)) / ds);
            ...
            selfRef.isMoved = true;
        }
    };
    d.onmouseup = function () {
        if (selfRef.isDrag.set && selfRef.isDrag.set.node.yesGraph && !selfRef.isMoved) {
            selfRef.isDrag.set.node.yesGraph.NodeClick(parseInt(selfRef.isDrag.set.node.id));
        }
        //selfRef.isDrag && selfRef.isDrag.set.animate({"opacity": 1}, 500);
        selfRef.isDrag = false;
        selfRef.isMoved = false;
    };
    this.draw();
};
Graph.Renderer.Raphael.prototype = {
    ...
    drawEdge: function(edge) {
        ...
        edge.connection.label.node.id = "l_" + edge.node + "_" + edge.port;
        edge.connection.bg.node.id = "p_" + edge.node + "_" + edge.port;
